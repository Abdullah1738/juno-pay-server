...
func (j *Junocashd) CLI(ctx context.Context, args ...string) ([]byte, []byte, error) {
	if j == nil || j.c == nil {
		return nil, nil, fmt.Errorf("junocashd container not started")
	}
	...
	if exitCode != 0 {
		return stdout.Bytes(), stderr.Bytes(), fmt.Errorf("junocash-cli exit %d", exitCode)
	}
...
func waitForRPCReady(ctx context.Context, jd *Junocashd) error {
	...
	for {
		_, _, err := jd.CLI(ctx, "getblockcount")
		if err == nil {
			return nil
		}
		select {
